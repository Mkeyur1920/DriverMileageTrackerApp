...
export class MileageRecordService {
  ...
  private readonly API_URL = `${API_BASE}/mileage-records`

  constructor(private http: HttpClient) {}

  getTotalKilometer(userId:number): Observable<number> {
  return this.http.get<number>(`${this.API_URL}/get-total-km/${userId}`, { observe: 'response' })
    .pipe(
  ...
  saveMileageRecord(startKm:any,endKm:any,totalKm:any,placesVisited:any,userId:any): Observable<boolean> {
    return this.http
      .post<MileageRecordDTO>(`${this.API_URL}/save`, { startKm,endKm,totalKm,placesVisited,userId }, { observe: 'response' })
      .pipe(
